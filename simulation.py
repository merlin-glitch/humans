...
# )
# from ressource import add_resource
# # ───────────────────── helpers ──────────────────────
# def _avg_pairwise_trust(ids: List[int], ts: TrustSystem) -> float:
#     n, s = 0, 0.0
#     L = len(ids)
...
#     # Zones de nourriture
#     img_path = os.path.join(os.path.dirname(__file__), "images", "5_spots_fixed.png")
...
from ressource import add_resource ,remove_resource
# ───────────────────── helpers ──────────────────────
def _avg_pairwise_trust(ids: List[int], ts: TrustSystem) -> float:
    n, s = 0, 0.0
    L = len(ids)
...
def run_simulation(
    ...
    # Zones de nourriture
    img_path = os.path.join(os.path.dirname(__file__), "images", "3_spots.png")
    zone_map, food_ids = identify_zones(img_path)
    zones = [extract_resource_coords_from_zones(zone_map, food_zone_id=zid)[0] 
            for zid in food_ids.values()]
    ...
    for tick in iterator:
        ...
        if ((tick - 1) % DAY_LENGTH) == 0:
            red_family = [h for h in humans if h.alive and h.home is houses[1]]
            if red_family:
                run_competition(red_family, trust_system, threshold=0.0)
