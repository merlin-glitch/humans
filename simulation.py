...
from ressource import (
    ...
    active_resources,  # returns a view of the active rows [x, y, life]
)
...
# ──────────────────────────────────────────────────────────────────────────────
# Adapter: present the NumPy store like a list so Human.step(...) keeps working
class _ResourceProxy:
    ...
    def update(self) -> None: pass         # aging handled centrally
    def is_alive(self) -> bool: return self.life > 0

class NumpyResourcesView:
    """Read-mostly, list-like wrapper over the NumPy resource buffer."""
...
def run_simulation(num_days: int):
    ...
    # seed initial resources (NumPy store; cap per cell by FOOD_STACK)
    food_zones = [(x, y) for y in range(H) for x in range(W) if codes[y, x] in {4, 5}]
    spawned = 0
    while spawned < INITIAL_FOOD_COUNT:
        x, y = random.choice(food_zones)
    ...
    # list-like view for Human.step compatibility
    ...
    for tick in trange(total_ticks, desc="Simulating", unit="tick"):
        ...
        # — update lifetimes & cull dead (vectorized)
        tick_resources_and_compact()
        ...
        for h in humans:
            if not h.alive:
                continue
            picked, shared = h.step(
                resources_view,  # adapter around NumPy store
        ...
        if tick and (tick % DAY_LENGTH) == 0:
            ...
            # populations
            blue_pop.append(sum(1 for hu in humans if hu.home is houses[0] and hu.alive))
            red_pop .append(sum(1 for hu in humans if hu.home is houses[1] and hu.alive))
