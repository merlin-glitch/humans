...
#     rev_color   = {
#         code: rgb
#         for rgb, code in map_draw.__globals__['NEW_PALETTE'].items()
#     }

#     # Houses (we assume exactly two: code 2=blue, code 3=red)
#     houses: List[House] = []
...
#         min_val=0.1, max_val=6.0, initial=1.0,
...
#     # Loop state
#     tick           = 0
#     day_tick       = DAY_LENGTH * 0.3  # start at 30% → morning
#     next_id        = humanid
...
#             # — DAWN? run exactly once per house —
#             # if not prev_is_day and is_day:
#             #     for house in houses:
#             #         family = [h for h in humans if h.home is house]
#             #         # only consider folks who have a memory_spot
#             #         candidates = [h for h in family if h.memory_spot]
#             #         if not candidates:
#             #             continue
#             #         leader = run_competition(candidates, trust_system)
#             #         print(f" House {house.x,house.y} leader = {leader.id}, spot = {leader.memory_spot}")
#             if not prev_is_day and is_day:
#                 for house in houses:
#                     family = [h for h in humans if h.home is house]
...
#             # 2a) DETECT DAWN: run competition once per house
#             # if not prev_is_day and is_day:
#             #     for house in houses:
#             #         family = [h for h in humans if h.home is house]
#             #         run_competition(family, trust_system)
#             #         print(f"leader for house ({house.x},{house.y}): {family[0]._last_night_leader}")
#             #         print(f"memory_spot for house ({house.x},{house.y}): {family[0].next_day_target}")
#             # prev_is_day = is_day
...
#             # 4) SLIDER‑DRIVEN SPAWN/CULL (day only on change)
#             if is_day and slider.value != prev_slider_val:
#                 target     = int(slider.value)
#                 num_houses = len(houses)
#                 base, extra= divmod(target, num_houses)
#                 desired    = {h: base + (1 if i < extra else 0)
#                               for i, h in enumerate(houses)}
#                 counts     = {h:0 for h in houses}
#                 for h in humans:
#                     counts[h.home] += 1
#                 for h_house, want in desired.items():
#                     have = counts[h_house]
#                     if have < want:
...
#                                 human_id=next_id,
#                                 sex=random.choice(['homme','femme']),
#                                 x=h_house.x, y=h_house.y,
#                                 home=h_house, codes=codes
#                             ))
#                             next_id += 1
#                     elif have > want:
#                         to_kill = have - want
...
#             # 6) HUMAN ACTIONS (now that competition has set next_day_target)
#             picked_this_tick = 0
#             for h in humans:
#                 if not h.alive:
...
#                     picked_this_tick += 1
#                 #the check if the food is picked way before going to check the food location
#                 if picked is not None:
#                     food_birth.pop(picked, None)
#                 if shared:
#                     total_shares += 1
...
#                  # 7) REWARD/PUNISH last night’s leader
...
#                     print(f"the leader {lid} is paid a share of {share} from {h.id} (leader now has {leader.bag})")
...
#                 # 8) MATING (within‐house only, with cooldown & energy check)
...
#             screen.blit(
#                 font.render(f"Shares: {total_shares}", True, (255, 255, 0)),
#                 (10, 10)
#             )
...
#     #export_house_contributions(humans, houses[0], "contributions_house1.csv", "contribution_to_house1")
#     #export_house_contributions(humans, houses[1], "contributions_house2.csv", "contribution_to_house2")
...
#     plot_population_variation(blue_file, red_file)
